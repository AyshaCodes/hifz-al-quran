...
export default function QuestionnaireContainer({ onComplete }: Props) {
        ...
        return (
          <Step1Situation
            ...
            onChange={v => {
              ...
              if (v === 'revision') {
                handleFieldChange('objectif', 'revision');
              }
            }}
          />
        );
      ...
      <div className="flex-1 flex flex-col items-center justify-start px-4 py-10 sm:py-16">
        <div className="w-full max-w-2xl">
          <div className="text-center mb-8">
            ...
            <h1 className="text-base font-semibold text-[#4a6b4b] tracking-wide uppercase text-sm">
              Mon Hifz
            </h1>
          </div>
          ...
          <Stepper
            ...
            hasMemorized={!isDebutant}
          />

          <div className="bg-white rounded-3xl shadow-sm border border-[#e8e4d4] p-6 sm:p-10 min-h-[360px] flex flex-col">
            <div className="flex-1">{renderStep()}</div>

            <div className="flex items-center justify-between mt-8 pt-6 border-t border-[#f0ece0]">
              <button
                onClick={handleBack}
                disabled={currentStep === 1}
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium text-[#7a8c7b] hover:text-[#2c3e2d] hover:bg-[#f0ece0] transition-all disabled:opacity-0 disabled:pointer-events-none"
              >
              ...
              <button
                onClick={handleNext}
                disabled={!canProceed()}
                className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold bg-[#2c6e3c] text-white hover:bg-[#235630] transition-all disabled:opacity-40 disabled:cursor-not-allowed shadow-sm hover:shadow-md"
              >
